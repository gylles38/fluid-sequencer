...
class Sequencer(EventDispatcher):
    ...
    def _calculate_song_length_in_beats(self) -> float:
        ...
        for track in self.song.tracks:
            if isinstance(track, AudioTrack):
                try:
                    # Note: Mute/Solo state is intentionally ignored here to make the length
                    # calculation static and cacheable. The trade-off is that auto-stop
                    # will be based on the absolute longest track, not the longest audible track.
                    with suppress_stdout_stderr():
