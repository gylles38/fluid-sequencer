...
###
    ...
    def _get_start_path_for_file_dialog(self, file_setting_key, dir_setting_key):
        """Determines the best starting directory for a file dialog."""
        # Prioritize the directory of the currently set file
        current_file = self.sequencer.config_manager.get_setting(file_setting_key)
        if current_file and os.path.exists(current_file):
            return os.path.dirname(current_file)

        # Fallback to the default directory for that type of file
        return self.sequencer.config_manager.get_setting(dir_setting_key)
    ...
    def set_carla_project_popup(self):
        ...
        start_path = self._get_start_path_for_file_dialog('default_carla_project_file', 'default_projects_dir')
    ...
    def set_aj_snapshot_popup(self):
        ...
        start_path = self._get_start_path_for_file_dialog('default_aj_snapshot_file', 'default_projects_dir')
    ...
    def add_midi_track_popup(self):
        ...
        content = BoxLayout(orientation='vertical', padding=dp(10), spacing=dp(10))
        track_name_input = TextInput(hint_text="Nom de la piste", size_hint_y=None, height=dp(40))
        content.add_widget(track_name_input)
        program_input = TextInput(hint_text="Programme MIDI (optionnel, 1-128)", size_hint_y=None, height=dp(40))
        ...
        buttons_layout.add_widget(cancel_button)
        content.add_widget(buttons_layout)
        popup = Popup(title="Ajouter une piste MIDI", content=content, size_hint=(0.6, None), height=dp(250), auto_dismiss=False)
    ...
    def add_audio_track_popup(self):
        ...
        name_popup = ConfirmationPopup(prompt_text="Entrez le nom de la nouvelle piste audio:", callback=on_name_confirm)
        name_popup.open()
    ...
    def delete_track_popup(self):
        ...
        for i, track in enumerate(tracks):
            btn = MDButton(MDButtonText(text=f"{i}: {track.name}"), size_hint_y=None, height=dp(40))
            btn.bind(on_release=lambda x, track_index=i: self.confirm_delete_track(track_index))
            grid.add_widget(btn)

        scroll_view.add_widget(grid)
        content.add_widget(scroll_view)
        popup = Popup(title="Sélectionner une piste à supprimer", content=content, size_hint=(0.7, 0.8))
    ...
    def confirm_delete_track(self, track_index):
        ...
        popup = YesNoPopup(prompt_text=f"Êtes-vous sûr de vouloir supprimer la piste '{track_name}'?", callback=on_confirm)
        popup.open()
    ...
    def start_play_blink(self):
        ...
        self.blink_animation = Clock.schedule_interval(self._toggle_play_button_color, 0.5)
    ...
    def start_recording_from_ui(self):
        ...
        for i, track in enumerate(self.sequencer.song.tracks):
            if isinstance(track, MidiTrack) and track.record_mode != 'OFF':
                if armed_track_index is not None:
                    self.show_error_popup("Multiple Tracks Armed", "Multiple tracks are armed for recording.\nPlease arm only one track.")
                    return False
                armed_track_index = i
        if armed_track_index is None:
            self.show_error_popup("No Track Armed", "No track is armed for recording.\nPlease arm a MIDI track first.")
            return False
        start_pos_text = self.start_pos_input.text.strip() or "1:1"
        ...
        if start_beat is None:
            ...
            return False
        try:
            result = self.sequencer.record_track(track_idx=armed_track_index, start_beat=start_beat, inport_name=self.sequencer.default_record_port)
            if "Error" in result:
    ...
    def toggle_track_mute(self, track_index):
        ...
        if 0 <= track_index < len(self.track_widgets):
            self.track_widgets[track_index].update_mute_solo_appearance()
    ...
    def on_start_position_validate(self, instance=None):
        ...
        self.sequencer.ui_start_pos_str = position
    ...
    def handle_position_arrows_in_textinput(self, textinput, direction, modifiers, cursor_pos):
        try:
            ...
            beat = max(1, min(beat, beats_per_measure))
    ...
    def process_command_ui(self, command):
        ...
        try:
            ...
            if data.get("status") == "file_chooser_prompt":
                def file_chooser_callback(filepath):
                    ...
                    self.process_command_ui(full_command)
                popup = FileChooserPopup(callback=file_chooser_callback, title="Select Audio File", filters=['*.wav', '*.mp3', '*.aiff', '*.ogg'])
                popup.open()
        ...
        lightweight_commands = ['play', 'pause', 'stop', 'loop', 'setloop', 'seek']
    ...
    def _beat_pulse_glow(self, dt):
        if not hasattr(self, 'beat_pulse_animation') or self.beat_pulse_animation is None:
            return
        self.pulse_phase += 1
        if self.pulse_phase <= 5:
            intensity = 0.3 + (self.pulse_phase * 0.14)
            ...
            if self.pulse_phase <= 10:
                intensity = 1.0 - ((self.pulse_phase - 5) * 0.14)
                glow_color = [intensity, 0.3 + intensity * 0.7, 0.1 + intensity * 0.3, 1]
                self.play_button.icon_color = glow_color
    ...
    def _synchronize_scroll(self, source_scroll_view, scroll_x_value):
        ...
        scrollable_widgets = [self.ruler.scroll_view] + [track.timeline_scroll for track in self.track_widgets if track.timeline_scroll]
