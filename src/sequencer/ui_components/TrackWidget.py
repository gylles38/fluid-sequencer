...
class TrackWidget(BoxLayout):
    ...
    # Propriétés pour l'alignement de la règle
    ...
    def __init__(self, track, track_index, sequencer_layout, **kwargs):
        ...
        self.info_section.bind(width=lambda instance, value: setattr(self, 'info_width', value))
        self.info_width = self.info_section.width  # Initial set

        # === 2. TIMELINE SECTION (MIDDLE) ===
        ...
        self.measure_grid = MeasureGrid(
            size_hint=(1, 1), beat_per_measure=4,
            total_beats=self.total_beats, pixels_per_beat=self.pixels_per_beat
        )
        ...
        self.controls_section = BoxLayout(orientation='horizontal', size_hint_x=None, spacing=dp(4))
        self.controls_section.bind(minimum_width=self.controls_section.setter('width'))

        # Track Type Icon
        type_icon_layout = self._create_type_icon_layout(track)
        ...
        if isinstance(track, MidiTrack):
            self.record_mode_button = ThreeStateRecordButton(
                ...
                sequencer_layout=sequencer_layout, callback=self.on_record_mode_change
            )
        ...
        # Solo Button
        self.solo_button = self._create_solo_button(track)
        self.controls_section.add_widget(self.solo_button)

        # MIDI Controls
        midi_controls_layout = self._create_midi_controls_layout(track)
        self.controls_section.add_widget(midi_controls_layout)

        # Volume Slider
        volume_layout = self._create_volume_layout(track)
        self.controls_section.add_widget(volume_layout)

        # Pan Slider
        pan_layout = self._create_pan_layout(track)
        self.controls_section.add_widget(pan_layout)

        self.add_widget(self.controls_section)
        self.controls_section.bind(width=lambda instance, value: setattr(self, 'controls_width', value))

        # Bind UI updates to property changes
        self.track.bind(is_solo=self.on_solo_changed)

    def _create_type_icon_layout(self, track):
        layout = BoxLayout(
            size_hint_x=None, width=dp(44), size_hint_y=None,
            height=dp(44), pos_hint={'center_y': 0.5}, padding=dp(4)
        )
        icon, color, bg_color = "help-circle", [0.5,0.5,0.5,1], [0.2,0.2,0.2,1]
        if isinstance(track, MidiTrack):
            icon, color, bg_color = "midi", [0.3,0.5,0.9,1], [0.2,0.3,0.4,0.3]
        elif isinstance(track, AudioTrack):
            icon, color, bg_color = "waveform", [0.9,0.5,0.2,1], [0.4,0.3,0.2,0.3]
        elif isinstance(track, AutomationTrack):
            icon, color, bg_color = "chart-line", [0.2,0.8,0.8,1], [0.2,0.4,0.4,0.3]
        ...
        layout.add_widget(MDIcon(
            icon=icon, theme_text_color="Custom", text_color=color,
            pos_hint={'center_x': 0.5, 'center_y': 0.5}
        ))
        return layout

    def _create_solo_button(self, track):
        return TooltipMDIconButton(
            icon='alpha-s-box' if track.is_solo else 'alpha-s-box-outline',
            tooltip_text='Solo' if not track.is_solo else 'Unsolo',
            on_press=self.on_solo_toggle, size_hint_x=None, width=dp(44),
            pos_hint={'center_y': 0.5}, theme_icon_color="Custom",
            icon_color=[1, 1, 0, 1] if track.is_solo else [0.6, 0.6, 0.6, 1],
            md_bg_color=[0.3, 0.3, 0.1, 0.8] if track.is_solo else [0.1, 0.1, 0.1, 0.8]
        )

    def _create_midi_controls_layout(self, track):
        layout = BoxLayout(size_hint_x=None, spacing=dp(8), pos_hint={'center_y': 0.5})
        layout.bind(minimum_width=layout.setter('width'))

        if isinstance(track, MidiTrack):
            # Channel
            channel_container = BoxLayout(orientation='horizontal', spacing=dp(4), size_hint_x=None)
            channel_container.bind(minimum_width=channel_container.setter('width'))
            channel_container.add_widget(Label(text='Ch:', size_hint_x=None, width=dp(25)))
            channel_spinner = ValueSpinner(
                min_val=1, max_val=16, initial_value=track.channel + 1,
                callback=self.on_channel_change, size_hint_x=None, width=dp(60)
            )
            channel_container.add_widget(channel_spinner)
            layout.add_widget(channel_container)

            # Program
            program_container = BoxLayout(orientation='horizontal', spacing=dp(4), size_hint_x=None)
            program_container.bind(minimum_width=program_container.setter('width'))
            program_container.add_widget(Label(text='Prog:', size_hint_x=None, width=dp(35)))
            program_spinner = ValueSpinner(
                min_val=1, max_val=128, initial_value=track.instrument + 1,
                callback=self.on_program_change, size_hint_x=None, width=dp(60)
            )
            program_container.add_widget(program_spinner)
            layout.add_widget(program_container)
        else:
            # Spacer to maintain alignment
            layout.add_widget(Widget(size_hint_x=None, width=dp(242)))
        return layout

    def _create_volume_layout(self, track):
        ...
        self.mute_button = TooltipMDIconButton(
            ...
            on_press=self.on_mute_toggle, size_hint_x=None, width=dp(32),
            pos_hint={'center_y': 0.5}, theme_icon_color="Custom",
    ...
    def _create_pan_layout(self, track):
        ...
        layout.add_widget(self.pan_slider)
        return layout
