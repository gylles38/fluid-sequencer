...
class PianoRoll(Widget):
    ...
    def draw(self, *args):
        ...
        with self.canvas:
            ...
            for i in range(129):
                line_y = round(i * self.note_height) + self.bottom_padding
                # Octave line (C)
                if (i % 12) == 0:
            ...
            for i in range(int(self.total_beats) + 1):
                ...
                y_grid_start = self.bottom_padding
                y_grid_end = self.height
                if i % self.beat_per_measure == 0:
                    major_vertices.extend([self.x + x_pos, self.y + y_grid_start, 0, 0, self.x + x_pos, self.y + y_grid_end, 0, 0])
                else:
                    minor_vertices.extend([self.x + x_pos, self.y + y_grid_start, 0, 0, self.x + x_pos, self.y + y_grid_end, 0, 0])
