...
class InternalRelativeLayout(RelativeLayout):
    _is_internal_widget = BooleanProperty(True)
...
<FloatingWindow>:
    ...
    canvas.after:
        # Border (In parent coordinates since StencilView is not translated)
        Color:
            rgba: 0.4, 0.4, 0.4, 1
        Line:
    ...
    # Internal RelativeLayout for coordinates.
    # It must be at (0,0) RELATIVE to this widget.
    InternalRelativeLayout:
        id: window_root
        size: root.size
        pos: root.pos[0] - root.pos[0], root.pos[1] - root.pos[1] # Always (0,0) relative to parent
        # Wait, size_hint: 1, 1 and pos: 0, 0 is enough for a child of a non-layout Widget (like StencilView)
        size_hint: 1, 1
        pos: 0, 0
...
class FloatingWindow(StencilView):
    ...
    def on_touch_down(self, touch):
        ...
        # local coordinates relative to window_root
        touch.push()
        touch.apply_transform_2d(self.ids.window_root.to_local)
        local_pos = touch.pos
    ...
    def on_touch_move(self, touch):
        ...
        if self._is_resizing:
            if self.parent:
                ...
                new_width = max(self.min_width, self._resize_start_widget_size[0] + dx)
        ...
        if not self.is_maximized:
            ...
            def finish_restore(dt):
                ...
                if self.size_hint[0] is None and self.size_hint[1] is None:
                    self.size = (min(self._restore_size[0], self.parent.width), min(self._restore_size[1], self.parent.height))
                if not self.pos_hint:
                    self.pos = (max(0, min(self._restore_pos[0], self.parent.width - self.width)), max(0, min(self._restore_pos[1], self.parent.height - self.height)))
